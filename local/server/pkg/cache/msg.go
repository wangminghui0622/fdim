...
// SeqProvider 序列号操作接口（可对接官?SeqConversationCache?
...
type RedisMsgCache struct {
	client      *redis.Client
	msgDB       interface{}
	seqProvider SeqProvider // 可选：委托给官?SeqConversation
}
...
// SetSeqProvider 设置 SeqProvider（对接官?SeqConversationCache，使 seq 操作走同一?Redis key?
func (c *RedisMsgCache) SetSeqProvider(sp SeqProvider) {
	c.seqProvider = sp
}

// getConversationMaxSeqKey 获取会话最大序列号?
...
// getConversationMinSeqKey 获取会话最小序列号?
...
// atomicGetAndDelOldSeq 原子地读取并删除?key，避免并发迁移竞?
...
func (c *RedisMsgCache) IncrMaxSeq(ctx context.Context, conversationID string) (int64, error) {
	if c.seqProvider != nil {
		...
		if newMax == 0 {
			...
			if oldSeq, ok := c.atomicGetAndDelOldSeq(ctx, conversationID); ok {
				// 通过 Malloc 预分?oldSeq ?seq，使新系?CURR 追上旧?
	...
	if err != nil {
		return 0, fmt.Errorf("递增序列号失? %w", err)
	}
	return newSeq, nil
}
...
func (c *RedisMsgCache) BatchInsertChat2Cache(ctx context.Context, conversationID string, msgs []*model.MsgDoc) (int64, bool, map[string]int64, error) {
	...
	for _, msg := range msgs {
		...
		msgBytes, _ := json.Marshal(msg)
		pipe.Set(ctx, msgKey, msgBytes, 7*24*time.Hour) // 7天过?
	...
	// 更新最大序列号
	pipe.Set(ctx, maxSeqKey, lastSeq, 0) // 不过?
...
func (c *RedisMsgCache) GetMessagesBySeq(ctx context.Context, conversationID string, seqs []int64) ([]*model.MsgDoc, error) {
	...
	for i, cmd := range cmds {
		...
		if err != nil {
			return nil, fmt.Errorf("获取消息序列?d失败: %w", seqs[i], err)
		}
...
func (c *RedisMsgCache) GetMaxSeq(ctx context.Context, conversationID string) (int64, error) {
	if c.seqProvider != nil {
		...
		// 新系统无数据，回退读旧 key（只读，不删除，迁移?IncrMaxSeq 触发?
