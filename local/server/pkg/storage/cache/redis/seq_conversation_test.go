...
func newTestSeq() *seqConversationCacheRedis {
	mgocli, err := mongo.Connect(context.Background(), options.Client().ApplyURI("mongodb://FDIM:FDIM123@127.0.0.1:37017/FDIM_v3?maxPoolSize=100").SetConnectTimeout(5*time.Second))
	if err != nil {
		panic(err)
	}
	model, err := mgo.NewSeqConversationMongo(mgocli.Database("FDIM_v3"))
	if err != nil {
		panic(err)
	}
	opt := &redis.Options{
