...
func newContentTypeConf(conf *config.Notification) map[int32]config.NotificationConfig {
	// 如果 conf 为 nil，返回空配置
	if conf == nil {
		return make(map[int32]config.NotificationConfig)
	}
...
func (s *NotificationSender) send(ctx context.Context, sendID, recvID string, contentType, sessionType int32, m proto.Message, opts ...NotificationOptions) {
	log.ZInfo(ctx, "[NotificationSender.send] called", "sendID", sendID, "recvID", recvID, "contentType", contentType, "sessionType", sessionType, "sendMsgNil", s.sendMsg == nil)
	// 如果sendMsg为nil，跳过发送
	if s.sendMsg == nil {
		log.ZWarn(ctx, "sendMsg is nil, skip notification", nil, "sendID", sendID, "recvID", recvID, "contentType", contentType)
		return
	}
	...
	req.MsgData = &msg
	log.ZInfo(ctx, "[NotificationSender.send] calling sendMsg RPC", "sendID", sendID, "recvID", recvID, "contentType", contentType, "sessionType", sessionType, "isSendMsg", optionsConfig.IsSendMsg)
	_, err = s.sendMsg(ctx, &req)
	if err != nil {
		log.ZWarn(ctx, "[NotificationSender.send] SendMsg RPC failed", err, "sendID", sendID, "recvID", recvID, "contentType", contentType)
	} else {
		log.ZInfo(ctx, "[NotificationSender.send] SendMsg RPC success", "sendID", sendID, "recvID", recvID, "contentType", contentType)
	}
}
