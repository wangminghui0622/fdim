...
// Load loads configuration from configDir/configName into the provided config structure.
// envPrefix is reserved for future environment variable override support.
func Load(configDir, configName, envPrefix string, config any) error {
	configPath := filepath.Join(configDir, configName)
	data, err := os.ReadFile(configPath)
	if err != nil {
		return errs.WrapMsg(err, "read config file error", "path", configPath)
	}
	if err = yaml.Unmarshal(data, config); err != nil {
		return errs.WrapMsg(err, "unmarshal yaml error", "path", configPath)
	}
	return nil
}
