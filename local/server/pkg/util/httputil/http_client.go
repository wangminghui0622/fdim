...
func (c *HTTPClient) Get(url string) ([]byte, error) {
	...
	if err != nil {
		return nil, errs.WrapMsg(err, "GET request failed: url=%s", url)
	}
	defer resp.Body.Close()

	body, err := io.ReadAll(resp.Body)
	if err != nil {
		return nil, errs.WrapMsg(err, "failed to read response body: url=%s", url)
	}
	return body, nil
}
...
func (c *HTTPClient) Post(ctx context.Context, url string, headers map[string]string, data any, timeout int) ([]byte, error) {
	...
	if data != nil {
		if err := json.NewEncoder(body).Encode(data); err != nil {
			return nil, errs.WrapMsg(err, "JSON encode failed")
		}
	}

	req, err := http.NewRequestWithContext(ctx, http.MethodPost, url, body)
	if err != nil {
		return nil, errs.WrapMsg(err, "NewRequestWithContext failed: url=%s method=%s", url, http.MethodPost)
	}
