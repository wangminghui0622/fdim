...
// MsgDoc 消息文档模型（对应 MongoDB 的 stream_msg 集合）
// 参考 OpenIM 官方实现，时间字段使用 int64 存储毫秒时间戳
type MsgDoc struct {
	...
	CreateTime       int64                  `bson:"create_time"`     // 毫秒时间戳
	SendTime         int64                  `bson:"send_time"`       // 毫秒时间戳
	...
	Ex               string                 `bson:"ex"`
	// 阅后即焚相关字段
	BurnDuration int32 `bson:"burn_duration,omitempty"` // 阅后即焚时长（秒），0表示不启用
	IsRead       bool  `bson:"is_read"`                 // 是否已读
	ReadTime     int64 `bson:"read_time,omitempty"`     // 阅读时间（毫秒时间戳，0表示未读）
