...
import (
	...
	"fdim/pkg/config"
	"fdim/pkg/db/cacheutil"
	"fdim/pkg/localcache"
	"fdim/pkg/localcache/lru"
	"fdim/pkg/log"
	"fdim/pkg/mcontext"
	"fdim/pkg/rpcli"
...
// parseUserOnlineStatus parses the user online status from redis message payload
// Format: "userID:platformID1,platformID2,..." or "userID:" (offline)
func parseUserOnlineStatus(payload string) (string, []int32, error) {
	...
	platformStrs := strings.Split(parts[1], ",")
	platformIDs := make([]int32, 0, len(platformStrs))
	for _, s := range platformStrs {
		...
		if err != nil {
			return "", nil, fmt.Errorf("invalid platformID: %s", s)
		}
		platformIDs = append(platformIDs, int32(id))
	}
