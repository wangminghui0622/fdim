...
func (f *FriendLocalCache) isFriend(ctx context.Context, possibleFriendUserID, userID string) (val *user.IsFriendResp, err error) {
	...
	var cache cacheProto[user.IsFriendResp]
	return cache.Unmarshal(f.local.GetLink(ctx, cachekey.GetIsFriendKey(possibleFriendUserID, userID), func(ctx context.Context) ([]byte, error) {
		...
		return cache.Marshal(f.client.FriendClient.IsFriend(ctx, &user.IsFriendReq{UserID1: userID, UserID2: possibleFriendUserID}))
...
// IsBlack possibleBlackUserID selfUserID.
func (f *FriendLocalCache) isBlack(ctx context.Context, possibleBlackUserID, userID string) (val *user.IsBlackResp, err error) {
	...
	var cache cacheProto[user.IsBlackResp]
	return cache.Unmarshal(f.local.GetLink(ctx, cachekey.GetIsBlackIDsKey(possibleBlackUserID, userID), func(ctx context.Context) ([]byte, error) {
		...
		return cache.Marshal(f.client.FriendClient.IsBlack(ctx, &user.IsBlackReq{UserID1: possibleBlackUserID, UserID2: userID}))
