...
func (g *GroupLocalCache) getGroupMemberIDs(ctx context.Context, groupID string) (val *user.GetGroupMemberUserIDsResp, err error) {
	...
	var cache cacheProto[user.GetGroupMemberUserIDsResp]
	return cache.Unmarshal(g.local.Get(ctx, cachekey.GetGroupMemberIDsKey(groupID), func(ctx context.Context) ([]byte, error) {
		...
		return cache.Marshal(g.client.GroupClient.GetGroupMemberUserIDs(ctx, &user.GetGroupMemberUserIDsReq{GroupID: groupID}))
	}))
}
