...
type Config struct {
	...
	// 离线推送配置
	Push struct {
		Enable            string `json:"Enable,optional"`            // getui, fcm, jpush, 或空（使用 dummy）
		FcmServerKey      string `json:"FcmServerKey,optional"`      // FCM 服务器密钥
		JPushAppKey       string `json:"JPushAppKey,optional"`       // 极光推送 AppKey
		JPushMasterSecret string `json:"JPushMasterSecret,optional"` // 极光推送 MasterSecret
	} `json:"Push,optional"`
}
