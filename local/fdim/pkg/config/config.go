...
func DefaultNotification() *Notification {
	reliable := NotificationConfig{IsSendMsg: true, ReliabilityLevel: 2} // ReliableNotificationNoMsg
	return &Notification{
		GroupCreated:              reliable,
		GroupInfoSet:              reliable,
		JoinGroupApplication:      reliable,
		MemberQuit:                reliable,
		GroupApplicationAccepted:  reliable,
		GroupApplicationRejected:  reliable,
		GroupOwnerTransferred:     reliable,
		MemberKicked:              reliable,
		MemberInvited:             reliable,
		MemberEnter:               reliable,
		GroupDismissed:            reliable,
		GroupMuted:                reliable,
		GroupCancelMuted:          reliable,
		GroupMemberMuted:          reliable,
		GroupMemberCancelMuted:    reliable,
		GroupMemberInfoSet:        reliable,
		GroupMemberSetToAdmin:     reliable,
		GroupMemberSetToOrdinary:  reliable,
		GroupInfoSetAnnouncement:  reliable,
		GroupInfoSetName:          reliable,
		FriendApplicationAdded:    reliable,
		FriendApplicationApproved: reliable,
		FriendApplicationRejected: reliable,
		FriendAdded:               reliable,
		FriendDeleted:             reliable,
		FriendRemarkSet:           reliable,
		BlackAdded:                reliable,
		BlackDeleted:              reliable,
		FriendInfoUpdated:         reliable,
		UserInfoUpdated:           reliable,
		UserStatusChanged:         reliable,
		ConversationChanged:       reliable,
		ConversationSetPrivate:    reliable,
	}
}
...
func (n *Nats) Build() *nats.Config {
	config := nats.DefaultConfig()
	config.Username = n.Username
	config.Password = n.Password
	config.Token = n.Token
	if n.MaxReconnects > 0 {
		config.MaxReconnects = n.MaxReconnects
	}
	if n.ReconnectWait > 0 {
		config.ReconnectWait = n.ReconnectWait
	}
	if n.Timeout > 0 {
		config.Timeout = n.Timeout
	}
	config.Addr = n.Address
	config.TLS = nats.TLSConfig{
		EnableTLS:          n.Tls.EnableTLS,
		CACrt:              n.Tls.CACrt,
		ClientCrt:          n.Tls.ClientCrt,
		ClientKey:          n.Tls.ClientKey,
		ClientKeyPwd:       n.Tls.ClientKeyPwd,
		InsecureSkipVerify: n.Tls.InsecureSkipVerify,
	}
	return config
}
...
func (n *Nats) GetConfigFileName() string {
	return NatsConfigFileName
}
