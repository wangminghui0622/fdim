...
############################## OpenIM Local Installation ##############################
# 
# This script has been modified to use LOCAL source code instead of downloading from GitHub.
# Make sure you're running this script from the OpenIM source code root directory.
#
# You can configure this script in three ways:
# 1. Set the variables in this section with more comments.
# 2. Pass an environment variable via a flag such as --help.
# 3. Set the variable externally, or pass it in as an environment variable

# Default configuration for OpenIM Local Installation
# The OpenIM settings can be customized according to your needs.

# Note: OWNER and REPO are kept for reference but not used for downloading
OWNER="OpenIMSDK" 
REPO="Open-IM-Server" 
...
# Note: This script now uses LOCAL source code instead of downloading from GitHub.
# Make sure you're running this script from the OpenIM source code root directory.

# GitHub proxy settings (for downloading Go, Docker images, etc.)
# If you are using a proxy, uncomment and replace the empty field with your proxy URL.
PROXY=
...
# check if the current directory is a OpenIM git repository or source code directory
function check_git_repo() {
    if git rev-parse --is-inside-work-tree >/dev/null 2>&1; then
        # Inside a git repository
        info "Detected git repository. Will use local source code."
        for remote in $(git remote); do
            repo_url=$(git remote get-url $remote)
            debug "Remote: $remote, URL: $repo_url"
        done
    else
        # Not a git repository, check if it's a source code directory
        if [ -f "go.mod" ] && [ -d "cmd" ] && [ -d "internal" ]; then
            info "Detected OpenIM source code directory. Will use local source code."
        else
            warn "Current directory is not a git repository or OpenIM source code directory."
            warn "Make sure you're running this script from the OpenIM source code root directory."
        fi
    fi
}
...
function download_source_code() {
    # Use local source code instead of downloading from GitHub
    info "Using local OpenIM source code"
    ...
    if ! git rev-parse --git-dir > /dev/null 2>&1; then
        warn "Not in a git repository. Checking if current directory contains OpenIM source code..."
        
        # Check for key OpenIM files
        if [ ! -f "go.mod" ] || [ ! -d "cmd" ] || [ ! -d "internal" ]; then
            fatal "Current directory does not appear to be an OpenIM source code directory."
            fatal "Please run this script from the OpenIM source code root directory."
            exit 1
        fi
        
        info "Using current directory as OpenIM source code directory: $(pwd)"
        success "Local source code detected"
        return 0
    fi
    
    # We're in a git repository, use current directory
    info "Detected git repository. Using current directory: $(pwd)"
    
    # Verify this is an OpenIM repository by checking for key files
    if [ ! -f "go.mod" ] || [ ! -d "cmd" ] || [ ! -d "internal" ]; then
        fatal "Current directory does not appear to be an OpenIM source code directory."
        fatal "Please run this script from the OpenIM source code root directory."
        exit 1
    ...
    success "Local source code verified and ready to use"
}
...
function cmd_help() {
    ...
    echo
    color_echo ${GREEN_PREFIX} "Note: This script uses LOCAL source code instead of downloading from GitHub.${COLOR_SUFFIX}"
    color_echo ${GREEN_PREFIX} "Make sure you're running this script from the OpenIM source code root directory.${COLOR_SUFFIX}"
    echo
    exit 1
}
...
O:::::::OOO:::::::O p:::::ppppp:::::::pe::::::::e            n::::n    n::::nII::::::IIM::::::M               M::::::M
 OO:::::::::::::OO  p::::::::::::::::p  e::::::::eeeeeeee    n::::n    n::::nI::::::::IM::::::M               M::::::M
   OO:::::::::OO    p::::::::::::::pp    ee:::::::::::::e    n::::n    n::::nI::::::::IM::::::M               M::::::M
    ...
    # Display product URL
    print_with_delay "Using local source code installation" 0.01
...
function openim_main() {
    # Check if we're in a valid OpenIM directory (git repo or source code)
    if ! git rev-parse --git-dir > /dev/null 2>&1; then
        # Not a git repo, check if it's a source code directory
        if [ ! -f "go.mod" ] || [ ! -d "cmd" ] || [ ! -d "internal" ]; then
            fatal "Please run this script from the OpenIM source code root directory."
            exit 1
        fi
    else
        # It's a git repo, verify it's OpenIM
        check_git_repo
    fi
